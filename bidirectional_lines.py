...
text="Image Width: 800 pixels"
font=cv2.FONT_HERSHEY_SIMPLEX
scale=1
thickness=1
text_width, text_height=cv2.getTextSize(text,font,scale,thickness)
image_width=image.shape[1]
text_x=(image_width-text_height)//3
text_y=text_height+230
...
cv2.imshow("Bidirectional Arrows", image)
cv2.waitKey(0)
cv2.destroyAllWindows()
cv2.imwrite("biderectional_arrows.png", cv2.cvtColor(image, cv2.COLOR_BGR2RGB))